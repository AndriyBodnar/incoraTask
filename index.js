...
function time() {
    ...
    if (year === new Date().getFullYear()) {
        ...
        if (month === new Date().getMonth() + 1) {
            ...
            releaseDate = `${randomNumber(1, 30)}-${month}-${year}`;
    }
    else
...
for (let i = 1; i <= 50; i++) {
    ...
    let service = streamingServices[randomNumber(0, streamingServices.length - 1)];
    let genre = Object.keys(genres)[randomNumber(0, Object.keys(genres).length - 1)];
